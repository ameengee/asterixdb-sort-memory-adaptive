...
VICTIM_INTERVAL=10; HEAP=4g; BUILD=1; JAR_OVERRIDE=""; PHASE_LOG=false; KWAY=false; CAPMULT=4; BUCKET_TUPLES=0; AUTOKEY=false; AUTOKEYINTS=3; TYPECUT=false; GCLOG=false; RELSHRINK=false; BUCKETCOUNT=256
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --bucket-count) BUCKETCOUNT="$2"; shift 2;;
...
JVM_ARGS="$JVM_ARGS -Dhyracks.sort.bucketCountTarget=$BUCKETCOUNT"
...
: > /tmp/.deploy_logsnap
for f in "$CLUSTER"/logs/nc-*.log; do
  [[ -f "$f" ]] && echo "$f $(wc -l < "$f")" >> /tmp/.deploy_logsnap
done
...
# Per-file offsets, captured AFTER the restart. Counting lines of `cat nc-*.log` and then tailing
# the re-concatenation is wrong once the files change independently -- an older deploy's
# `policy=none` leaks through, and the confirmation then rejects a correct deploy (this broke the
# master.jar arm of the merge-pass run).
ACTUAL=$(while read -r f n; do tail -n +$((n+1)) "$f"; done < /tmp/.deploy_logsnap 2>/dev/null \
